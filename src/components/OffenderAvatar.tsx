...
// README), else an initials avatar - same fallback pattern used by
...
export default function OffenderAvatar({
  ...
  if (photoUrl) {
    ...
    // would need those declared. A handful of small avatar images doesn't
    // need the optimizer.
    return (
      <img
        src={photoUrl}
  ...
  return (
    <span
      className={`flex shrink-0 items-center justify-center rounded-full font-semibold ${c.bg} ${c.text}`}
